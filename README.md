...
| --- | --- | --- | --- |
| `v0.2.1` | 2026-03-13 | **Doc-only patch**: simplified install/upgrade paths — removed `/gov_help` from Fastest Operator Entry and removed manual chain step 5 from New Install and Upgrade paths; `/gov_setup quick` is now the single recommended entry point. Bootstrap payload synced. | Cleaner quickstart — operators no longer see redundant step-by-step manual chain in the recommended path. |
